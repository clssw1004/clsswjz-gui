...
class SyncService extends BaseService {
  ...
  Future<void> syncChanges(
      {Function(int percent, String message)? onProgress}) async {
    ...
    if (clientChanges.isNotEmpty) {
      await _processOnProgress(
          onProgress, 10, '本地变更：${clientChanges.length}，开始同步本地变更到服务端');
    } else {
      await _processOnProgress(onProgress, 10, '本地无变更');
    }
    final syncResult = await _syncChangesToServer<SyncResponseDTO>(
        clientChanges, lastSyncTime);
    ...
    if (resultLogs.isNotEmpty) {
      ...
      await _processOnProgress(onProgress, 30,
          '服务端同步完成：${resultLogs.length}，成功：$success，失败：$failed');
      await _syncLogState(resultLogs, syncResult.syncTimeStamp);
    }
    if (syncResult.changes.isNotEmpty) {
      await _processOnProgress(
          onProgress, 40, '服务端变更：${syncResult.changes.length}，开始同步到本地');
  ...
  Future<void> _syncLogState(
      List<SyncResultDTO> results, int syncTimestamp) async {
  ...
  /// 同步本地变更到服务端
  Future<SyncResponseDTO> _syncChangesToServer<T>(
      List<LogSync> logs, int syncTimeStamp) async {
  ...
  Future<void> _processOnProgress(
      Function(int percent, String message)? onProgress,
      int percent,
      String message) async {
    if (onProgress != null) {
      onProgress(percent, message);
    }
    print('progress: $percent, message: $message');
  }
}
