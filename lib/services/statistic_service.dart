...
class StatisticService {
  ...
  Future<OperateResult<UserStatisticVO>> getUserStatisticInfo(String userId) async {
    ...
    final days = await (db.select(db.accountItemTable)..where((tbl) => tbl.createdBy.equals(user.id)))
        .get()
        .then((items) => items.map((e) => e.accountDate.toString().split(' ')[0]).toSet().length);
